...
public:
	static int ExecuteQueryForInsert(sql::PreparedStatement* pPreparedStatement, const CUSER_INFO& data) {
		try {
			if (pPreparedStatement) {
				pPreparedStatement->setInt(1, data.m_token.m_rawData);
				pPreparedStatement->setString(2, data.m_user_id.m_rawData);
				pPreparedStatement->setString(3, data.m_uesr_pw.m_rawData);
				pPreparedStatement->setString(4, data.m_user_name.m_rawData);

				return pPreparedStatement->executeUpdate();
			}
		}
		catch(const sql::SQLException& exce) {
			SERVER::FUNCTIONS::LOG::Log::WriteLog(L"SQL Error - %d", exce.getErrorCode());
			return exce.getErrorCode();
		}
		return -1;
	}

	static bool ExecuteQueryForSelect(sql::PreparedStatement* pPreparedStatement, std::vector<CUSER_INFO>& listOfOutput) {
		try {
			if (pPreparedStatement) {
	...
	static bool ExecuteQueryForDelete(sql::PreparedStatement* pPreparedStatement) {
		try {
			if (pPreparedStatement) {
...
public:
	...
	static sql::PreparedStatement* PreparedQueryForSelect(sql::Connection* sqlRealConnection, const std::vector<SERVER::FUNCTIONS::MYSQL::SQL::CQueryWhereConditional>& listOfConditional) {
		try {
			return sqlRealConnection->prepareStatement(CBaseTable::MakeQueryForSelect(user_info_TABLE_NAME, std::vector<std::string>{}, listOfConditional)); 
		}
		catch (const sql::SQLException& exce) {
			SERVER::FUNCTIONS::LOG::Log::WriteLog(L"SQL Error - %d", exce.getErrorCode());
			return nullptr;
